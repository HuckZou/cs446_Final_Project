...
if __name__ == "__main__":
	...
	# Find the last time data was updated and increment it by one

	query = "select MAX(price_date) from daily_Price"
	...
	start = 19
	end = start_date.index(')')
	start_date = start_date[start:end].split(', ')
	...
	start_date = start_date.timetuple()[0:3]

	for t in tickers:
		print "Adding data for %s" % t[1]
		yf_data = get_daily_historic_data_yahoo(t[1], start_date)
		if not yf_data:
